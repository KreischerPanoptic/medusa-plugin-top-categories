...
export const GET = async (
    ...
    let results = expand ? await categoriesService.listAndCount({}, {take: parseInt(`${limit || '1000'}`), skip: parseInt(`${offset || '0'}`), relations: [ `${expand}` ]}) : await categoriesService.listAndCount({}, {take: parseInt(`${limit || '10'}`), skip: parseInt(`${offset || '0'}`)})
    let categories = []
