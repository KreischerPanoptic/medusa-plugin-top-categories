...
export const GET = async (
    ...
    const categoriesService: ProductCategoryService = req.scope.resolve(
      ...
      let result: [ProductCategory[],number] = expand ? await categoriesService.listAndCount({handle: handle}, {take: 1, skip: 0, relations: [ `${expand}` ]}) : await categoriesService.listAndCount({handle: handle}, {take: 1, skip: 0});

      if (result[1]<=0) {
        throw new MedusaError(
          MedusaError.Types.NOT_FOUND,
          `Product category with handle: ${handle} was not found`
        )
      }
      ...
      category = await categoriesService.update(category.id, {
